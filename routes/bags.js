...
router.post('/upgradePoke', async(req, res) => {
    ...
    var usr = await User.findOne({ _id: req.user._id })
        .populate({
            path: 'bag',
    ...
    if(check === 0){
        ...
        res.status(200).json({ messenger: 'Tiến hóa thành công !' })
    }
...
//     })
// })
router.post('/allPoke', (req, res) =>{
    Pokemon.find({ typePokemons: req.body.idtype })
            .exec((err, poke)=>{
                res.send(poke);
            })
})
